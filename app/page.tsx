...
export default function HomePage() {
  ...
  const activeDebtCount = useMemo(() => {
    return debts.filter((item) => toSafeNumber(item.remaining_debt) > 0).length;
  }, [debts]);
  ...
  return (
    <main className="min-h-screen bg-gray-50 p-6">
      ...
      <div className="mx-auto max-w-7xl space-y-6">
        ...
        <PageHeader
          title="Borç Takip Sistemi"
          subtitle={`Borç, kasa ve ödeme takibi kullanıcı hesabı kapsamında çalışır. ${getAuthModeLabel(authContext)}.`}
        />
